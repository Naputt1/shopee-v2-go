...
type ResponseDataShippingDocumentInfo struct {
	...
	IsLmDgBool                   int64                   `json:"is_lm_dg_bool"`                   // [Required] <p>Use this field to indicate order contains dangerous goods or not.</p><p>0: Non-dangerous good</p><p>1: Dangerous good</p><p>2: Prohibited</p>
	...
	HighValue                    bool                    `json:"high_value"`                      // [Required] <p>This value indicates whether the order is considered a “high value” item and requires special handling by the logistics provider. The threshold to be considered "high value" item differs by region, and is only applicable to SPX channels. For regions other than Malaysia and Thailand, this field will always return empty.</p>
	DgSpecificType               int64                   `json:"dg_specific_type"`                // [Required] <p>Currently only applicable for&nbsp;Brazil, Indonesia, Vietnam, Philippines.</p><p>For orders with Dangerous Goods, this value indicates the severity of the danger and requires special handling by the logistics provider.&nbsp;</p><p><br /></p><p>0 = Not classified / no DG sub-type</p><p>1 = DG_A</p><p>2 = DG_B</p><p>3 = DG_C</p><p>4 = DG_D</p>
	HotspotId                    string                  `json:"hotspot_id"`                      // [Required] <p>This ID is used by 3PL to determine parcel routing. For regions other than Malaysia and Thailand, this field will always return empty.</p>
	Weekend1DeliverySuccessLabel string                  `json:"weekend1_delivery_success_label"` // [Required] <p>This value indicates whether an order has a high / medium / low delivery success rate on each weekend (e.g. sat and sun respectively).&nbsp;For regions other than Malaysia, this field will always return empty.</p><p>High = H</p><p>Medium = M</p><p>Low = L</p>
	Weekend2DeliverySuccessLabel string                  `json:"weekend2_delivery_success_label"` // [Required] <p><br />This value indicates whether an order has a high / medium / low delivery success rate on each weekend (e.g. sat and sun respectively).&nbsp;For regions other than Malaysia, this field will always return empty.</p><p>High = H</p><p>Medium = M</p><p>Low = L</p>
}
